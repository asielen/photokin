...
| Claude Code CLI | `claude-code` (alias: `claude_code`) | same as Anthropic (`--claude-model` / `CLAUDE_MODEL`) | _none — local `claude` CLI login instead, see `utils.claude_code_cli_status`_ |
...
Claude aliases resolve via `CLAUDE_MODELS` in `utils.py` (`sonnet` → `claude-sonnet-5`, `haiku` → `claude-haiku-4-5-20251001`; default alias `sonnet`). These are Anthropic's own undated aliases, each tracking the newest snapshot of its generation on the direct API — a fully bare `claude-sonnet`/`sonnet` is not a model the API accepts, so photokin always resolves to one of these full names before calling it. The one place prompt content does vary by provider: Gemini gets an extra JSON-syntax guardrail appended, paired with Gemini-specific JSON repair in `utils._cleanup_model_json` — it earned both.

"Shared everything upstream" still leaves real differences underneath, and they occasionally matter when debugging:

- **OpenAI**: Responses API; `temperature=0` where the model supports it (omitted for o-series and gpt-5+). No `max_output_tokens` ceiling is set — omitting it uses the model's own default rather than a photokin-chosen cap low enough to realistically truncate. An `incomplete`/`max_output_tokens` response still raises a clean `length` error rather than failing to parse, if a model's own limit is ever hit regardless. Archival `files.create` uploads run only for the OpenAI provider.
- **Claude**: Messages API via streaming (the SDK refuses long non-streaming calls). `max_tokens=16384`, or a 64k output budget when extended thinking is enabled (adaptive thinking; Haiku gets a manual budget instead). Even without `thinking` requested, a model can spend part of the 16384 on its own internal reasoning before any answer text exists (observed on Sonnet 5) — a response that truncates (`stop_reason=max_tokens`) gets one retry at the 64k ceiling before it's reported as a `length` error. `temperature=0` only on models that still accept it — Opus 4.7+, Sonnet 5, and the Fable/Mythos family reject the parameter outright. Prompt parts are concatenated into one text block; images become base64 image blocks (MIME must be `image/jpeg`, `image/png`, `image/gif`, or `image/webp`), ordered images-first.
- **Gemini**: `generate_content` with `response_mime_type=application/json`, `temperature=0`, `max_output_tokens=16384`. Same one-retry-at-64k-then-`length`-error shape as Claude for a `finish_reason=MAX_TOKENS` response; images become `inline_data` parts. Gemini has no per-model naming heuristic for temperature support the way OpenAI/Claude do (nothing in a Gemini model name signals it), so an unsupported override is detected reactively instead: a request rejected for it is retried once without `temperature`.
- **OpenRouter**: Chat Completions (`/v1/chat/completions`) via the `openai` SDK with `base_url=https://openrouter.ai/api/v1` (override with `OPENROUTER_BASE_URL` to point at any other OpenAI-compatible gateway). `max_tokens=16384` — reasoning models like Kimi spend part of the budget thinking before they write, and a smaller cap truncates them mid-answer. Same one-retry-at-64k-then-`length`-error shape as Claude/Gemini for a `finish_reason=length` response, and the same reactive temperature retry as Gemini (no per-slug naming heuristic makes sense across every model OpenRouter fronts). `temperature=0`; images become `image_url` data-URL parts, text-first. Archival file uploads are skipped (OpenAI-only). One field note: a model page saying "multimodal" does not guarantee the endpoint OpenRouter routes you to accepts images — test a couple of photos before committing a big batch to an unfamiliar slug.
- **Claude Code CLI** (`api_claude_code.py`): no SDK, no HTTP client — `_build_provider_client` instead probes `claude_code_cli_status()` (`shutil.which("claude")` + `claude auth status`) and, on success, hands the adapter a `SimpleNamespace(binary_path=...)`. Each call spawns `claude --bare -p --system-prompt "" --disallowed-tools "*" --model <model> --input-format stream-json --output-format stream-json --verbose` (`--verbose` is required by the CLI whenever `--print` pairs with `--output-format=stream-json`), feeding one `{"type": "user", "message": {"role": "user", "content": [...]}}` line on stdin — the same image/text content-block shape the Messages API uses, confirmed by direct trial against the CLI (image *handling*, as opposed to acceptance, still wants end-to-end confirmation against a real logged-in CLI). Reads the NDJSON events back off stdout, keeping the last `type: "result"` event; `is_error` on that event raises a `ProviderApiError`, classified by matching substrings in the free-text `result` message (`"auth"`/`"login"` → `missing_api_key`/401, `"rate limit"`/`"usage limit"` → `rate_limit`/429, `"overloaded"` → `overloaded`/529, else `api_status`) since the CLI exposes no structured error taxonomy the way the SDKs' typed exceptions do. No `temperature`, and `thinking` is accepted for interface parity but currently a no-op. Billed against the operator's Claude subscription usage rather than a metered API key; subject to the CLI's own rolling usage limits rather than Messages API rate limits, and each call's subprocess spawn adds overhead a persistent streaming client doesn't have. Never auto-selected (no SDK for `installed_provider_sdks()` to find) — always an explicit `--provider claude-code` / `LLM_PROVIDER=claude-code`.

A truncated response's single retry re-sends the whole request (including any images) at the higher ceiling — there is no cheaper "continue where it left off" without also implementing prompt caching, so this is deliberately a *rare* escalation (the base ceilings above are sized to cover the common case) rather than the default behavior. Raising the base ceiling instead of retrying was considered and rejected: providers only bill for tokens actually generated, not the ceiling, so the common case costs the same either way, while a retry-from-scratch on top of an already-generous base would rarely trigger at all.
...
One field of the record is a property of the **group** rather than of the file it is keyed under — for a group of views of one object, which is most groups. `caption` is a single labelled block built from every file's own existing caption plus this run's own transcription, and every record in the group carries it byte-identically:
...
**A multipage group (`multipage_present`) does not follow that rule.** An ordered sequence of pages is not a group of views sharing one object, so each file's `caption` instead carries only its own part's transcription — resolved via `core.resolve_part_label`, the same function the sidecar uses. Two records of a multipage group are byte-identical only when they are variant scans of the same part, which still share one transcription exactly as they always have; do not read caption equality as "these are the same page" in the other direction, because a fallback record (below) can match another fallback record too. It carries no label: the file holds exactly one part's text, so there is nothing to tell it apart from. When this file's part is missing from the reply's `transcriptions` (no map at all, or a displaced/unseated file that never rode the payload under any label), the record falls back to the group's shared block instead, built exactly as it always was.

**`caption_scope`** names which of the two a given record got: `"part"` for a file that received its own page's text, `"group"` for one that fell back to the shared block. The key is present only on a record from a multipage group — everywhere else the caption is group-scoped by design and the key would only repeat that. It describes what **this run attributed**, not what the stored caption now contains — those differ on an archive processed before 0.5.0, where the file already held the whole-document block: the run attributes its page, writes `"part"`, and the old block is still in there beside it, because a caption is merged and nothing this tool wrote is discarded. So `caption_scope` is not a test for "this caption is one page's text"; it answers only "did this run have a transcription of its own to give this file". Read `caption_scope`, not caption length or content, to tell the two regimes apart; a folder re-run after upgrading past 0.5.0 can genuinely hold both at once (an archive processed before the change keeps its old group-wide caption on every file, permanently — see "Documents get their own page, not the whole book" in the root [README](../README.md#captions)), and this key is how a consumer tells that state apart from an ordinary group-of-views record.

Two consequences for an embedder, both scoped to a group of views of one object — see above for how a multipage group differs. A caption your hydrator supplies for one item will appear in its siblings' records — that is deliberate, and `--group-by none` is the way to opt out of it. And the block is designed to be fed back in: pass a previous run's `caption` to the next run's hydrator and it is recognised, not re-labelled and not appended to when the model reads the same text the same way again, so a repeated enrichment pass is a fixed point rather than unbounded growth. The block's shape, the label rules and the merge rules are documented for users in the root [README](../README.md#captions); the record's own caption is what the write-back applies to `XMP-dc:Description`, so an embedder that renders records itself sees the same text.

**`hydration_failed`** appears on a record (as `true`) when `-r` asked for that file's metadata and the read could not be confirmed — the batch read failed, or ExifTool returned no record or an error for the file. Such a record's `patch` is deliberately empty and its changeset entry proposes nothing: unread is not empty, and a write instruction diffed against an unseen before-snapshot could overwrite metadata the file really holds. An integration that applies patches through its own writer (the Lightroom plug-in) must treat the empty patch as final rather than reconstructing writes from `result`. The key is absent everywhere else. The file's changeset record carries the same verdict as `"suppressed": "hydration_failed"` inside `proposed_changes`, which is how the applier counts suppressed records — a write run whose every record was suppressed exits 2 rather than reporting success.
